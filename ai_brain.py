...
class OFIV5SniperEngine:
    ...
    def execute_trade(self, side, quantity, current_price, tp_pct, sl_pct, z_ofi, strategy, spread, vol):
        ...
        # Step 1: Market Order (Entry) with idempotency key
        import uuid
        client_order_id = f"koda_{int(time.time())}_{uuid.uuid4().hex[:8]}"
        print(f"[>] EXECUTING {side} {quantity} at {current_price} (id: {client_order_id})")
        main_res = self.execute_binance_request("POST", "/fapi/v1/order", {
            "symbol": self.symbol.upper(), "side": side, "type": "MARKET", 
            "quantity": quantity, "newClientOrderId": client_order_id
        })
        
        # Validate order was actually filled before continuing
        ...
        # Slippage guard: check actual fill price vs expected
        raw_avg = main_res.get('avgPrice', '0')
        fill_price = float(raw_avg) if raw_avg and float(raw_avg) > 0 else 0
        if fill_price == 0:
            # Edge case: avgPrice missing/zero — flag as unknown fill, skip slippage check
            print(f"    [?] avgPrice unavailable, marking UNKNOWN_FILL. Proceeding with caution.")
            self.chat_log.append({"s": "RISK_ENGINE", "c": "risk", "m": "[UNKNOWN_FILL] avgPrice=0 from exchange. Slippage check skipped."})
            fill_price = current_price  # Use expected price as fallback
            slippage = 0
        else:
            slippage = abs(fill_price - current_price) / current_price
        slippage_limit = self.live_cfg.get('slippage_guard_pct', 0.0008)
        if slippage > slippage_limit:
            print(f"    [!] SLIPPAGE {slippage*100:.4f}% > limit {slippage_limit*100:.2f}% — closing position immediately!")
            ...
            self.chat_log.append({"s": "RISK_ENGINE", "c": "risk", "m": f"[SLIPPAGE GUARD] {slippage*100:.4f}% — position closed."})
            return {"status": "SLIPPAGE_REJECT", "slippage": slippage}
        
        print(f"    [OK] Entry filled @ {fill_price} (slip: {slippage*100:.4f}%). OrderId: {main_res.get('orderId')}")
        ...
        if signal and time.time() - self.last_trade_time > 10:
            ...
            def execute_and_log():
                ...
                if isinstance(res, dict) and 'orderId' in res:
                    ...
                    self.chat_log.append({"s": "EXECUTION", "c": "risk", "m": f"[FAILED] Trade not placed: {res}"})
            ...
            try:
                async with websockets.connect(url) as ws:
                    while True:
                        ...
                        # EVENT 1: ACCOUNT_UPDATE — tracks real position state (Codex fix)
                        if data.get('e') == 'ACCOUNT_UPDATE':
                            ...
                            for pos in positions:
                                if float(pos.get('pa', 0)) != 0:
                                    active_count += 1
                            self.open_positions = active_count
                            print(f"    [POS SYNC] open_positions = {self.open_positions} (from ACCOUNT_UPDATE)")
                        
                        # EVENT 2: ORDER_TRADE_UPDATE — tracks PnL on close fills
    ...
    def sync_open_positions(self):
        """Fallback: poll Binance REST API to sync open_positions with exchange truth."""
        try:
            res = self.execute_binance_request("GET", "/fapi/v2/positionRisk", {})
            if isinstance(res, list):
                active = sum(1 for p in res if float(p.get('positionAmt', 0)) != 0)
                if active != self.open_positions:
                    print(f"    [POS SYNC REST] Corrected open_positions: {self.open_positions} -> {active}")
                    self.open_positions = active
        except Exception as e:
            print(f"    [POS SYNC REST] Failed: {e}")

    async def position_sync_loop(self):
        """Periodic position sync every 60s as safety net."""
        while True:
    ...
    async def daily_reset_loop(self):
        """Reset daily counters at UTC 00:00."""
        import datetime
        while True:
            now = datetime.datetime.utcnow()
            tomorrow = now.replace(hour=0, minute=0, second=0, microsecond=0) + datetime.timedelta(days=1)
            wait_seconds = (tomorrow - now).total_seconds()
            print(f"[DAILY RESET] Next reset in {wait_seconds/3600:.1f}h (UTC 00:00)")
    ...
    async def main(self):
        ...
        task4 = asyncio.create_task(self.position_sync_loop())
        task5 = asyncio.create_task(self.daily_reset_loop())
